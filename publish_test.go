...
func TestPublishCmd_ExitQuietly_JSON(t *testing.T) {
	...
	retErr := cmd.exitQuietly(globals, "outside_schedule")
...
func TestPublishCmd_ExitQuietly_Human(t *testing.T) {
	// In human mode, exitQuietly should produce no output and return nil.
	oldStdout := os.Stdout
	r, w, err := os.Pipe()
	require.NoError(t, err)
	os.Stdout = w
	...
	retErr := cmd.exitQuietly(globals, "no_queued")
	...
	assert.NoError(t, retErr)
	assert.Equal(t, 0, n, "expected no output in human mode")
}
...
func alwaysActiveSchedule() config.Schedule {
...
func neverActiveSchedule() config.Schedule {
	return config.Schedule{
		PostEveryMinutes: 0,
		StartHour:        0,
		EndHour:          0,
...
func TestPublish_HappyPath(t *testing.T) {
	...
	output := captureStdout(t, func() {
		retErr := cmd.publishOne(srv.URL, cfg, globals)
		assert.NoError(t, retErr)
	})
...
func TestPublish_OutsideHours(t *testing.T) {
	...
	output := captureStdout(t, func() {
		retErr := cmd.publishOne("http://unused", cfg, globals)
		assert.NoError(t, retErr)
	})
...
func TestPublish_TooSoon(t *testing.T) {
	...
	cfg := config.Config{Schedule: config.Schedule{
