...
func (cmd *PostCmd) Run(globals *Globals) error {
	...
	// 8. Warn if schedule not installed.
	scheduleWarning := ""
	if !launchd.IsInstalled() {
		scheduleWarning = "Schedule not configured — run `slack-social-ai schedule install` to enable automatic publishing."
	}
