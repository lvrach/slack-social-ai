...
func TestCLI_ScheduleStatusNotConfigured(t *testing.T) {
	...
	// No config file in temp HOME → not configured.
	assert.Contains(t, stdout, "Not configured", "should say not configured when no config exists")
}
...
func TestCLI_ScheduleStatusNotConfiguredJSON(t *testing.T) {
	...
	err := json.Unmarshal([]byte(strings.TrimSpace(stdout)), &resp)
	require.NoError(t, err, "stdout should be valid JSON")
	assert.Equal(t, "not_configured", resp["status"], "JSON status should be 'not_configured' when no config exists")
}
...
func TestCLI_PostNotConfigured(t *testing.T) {
	...
	assert.Contains(t, stderr, "auth login", "error should mention running auth login")
}
...
func TestCLI_Help(t *testing.T) {
	...
	assert.Contains(t, stdout, "auth", "help should mention the auth command")
}

// --- auth status command (not configured in temp HOME) ---

func TestCLI_AuthStatusNotConfigured(t *testing.T) {
	stdout, _, exitCode := runCLI(t, "auth", "status")

	assert.Equal(t, 0, exitCode, "auth status should exit 0")
	assert.Contains(t, stdout, "not configured", "should say not configured")
}

func TestCLI_AuthStatusNotConfiguredJSON(t *testing.T) {
	stdout, _, exitCode := runCLI(t, "auth", "status", "--json")

	assert.Equal(t, 0, exitCode, "auth status --json should exit 0")

	var resp map[string]any
	err := json.Unmarshal([]byte(strings.TrimSpace(stdout)), &resp)
	require.NoError(t, err, "stdout should be valid JSON")
	assert.Equal(t, false, resp["configured"], "should show configured: false")
}

// --- auth logout command (not configured) ---

func TestCLI_AuthLogoutNotConfigured(t *testing.T) {
	stdout, _, exitCode := runCLI(t, "auth", "logout")

	assert.Equal(t, 0, exitCode, "auth logout should exit 0 when no credentials")
	assert.Contains(t, stdout, "No webhook credentials", "should report no credentials")
}
